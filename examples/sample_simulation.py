...
async def main(loop):
    ...
    async def update_callback(msg):
        print(f"Sent update")
        await nats.publish(msg.reply, numpy_encode(np.random.randint(0, 2, 256).astype(dtype=np.float64)))

    async def reward_callback(msg):
        print(f"Sent reward")
        await nats.publish(msg.reply, numpy_encode(np.random.randint(0, 10, 1).astype(dtype=np.float64)))

    async def action_callback(msg):
        print(f"Taking action {numpy_decode(msg.data)}")
    ...
    async def actions_callback(msg):
        ...
        await nats.publish(msg.reply, numpy_encode(np.random.randint(0, 1, 11).astype(dtype=np.float64)))

    async def done_callback(msg):
        print(f"Sent done")
        await nats.publish(msg.reply, numpy_encode(np.random.randint(-3, 3, 12).astype(dtype=np.float64)))
